...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the shared GUI client; [`apps/web`](../apps/web) publishes it to browsers, [`apps/electron`](../apps/electron) publishes it as a macOS desktop application without an HTTP listener, and [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all.
