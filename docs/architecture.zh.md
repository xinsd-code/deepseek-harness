...
[`dsh-base`](../packages/bundle/base/README.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-web-app`](../packages/bundle/web-app/README.md) 增加共享 GUI 客户端；[`apps/web`](../apps/web) 将它发布给浏览器，[`apps/electron`](../apps/electron) 将它发布为不监听 HTTP 端口的 macOS 桌面应用，而 [`dsh-headless`](../packages/bundle/headless/README.md) 增加一次性运行器，且完全不带服务器。
