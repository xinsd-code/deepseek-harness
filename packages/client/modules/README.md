...
The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, and hashes the built bundle into the boot graph. With the default `web` carrier it publishes bundles and source maps under `/plugins` and injects that graph into the HTML; with the `electron` carrier the desktop shell reads the same registry and publishes both through its local custom protocol. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
